...
class CadastroFisioterapeuta(BaseFrame):
    ...
    def __init__(self, master, voltar_callback):
        ...
        self.label_id.grid(row=1, column=1, sticky="e", padx=(20,10), pady=(20,10))
        
        self.entry_id = self.widgets.entry(self, None)
        self.entry_id.grid(row=1, column=2, sticky="w", padx=10, pady=(20,10))

        self.label_nome = self.widgets.label(self, texto="Nome:", cor="transparent")
        self.label_nome.grid(row=1, column=3, sticky="e", padx=10,pady=(20,10))

        self.entry_nome = self.widgets.entry(self, None)
        self.entry_nome.grid(row=1, column=4, sticky="w", padx=(10,20), pady=(20,10))

        self.label_login = self.widgets.label(self, texto="Login:", cor="transparent")
        self.label_login.grid(row=2, column=1, sticky="e", padx=(20,10), pady=(20,10))

        self.entry_login = self.widgets.entry(self, None)
        self.entry_login.grid(row=2, column=2, sticky="w", padx=10, pady=(20,10))

        self.label_senha = self.widgets.label(self, texto="Senha:", cor="transparent")
        self.label_senha.grid(row=2, column=3, sticky="e", padx=10, pady=(20,10))

        self.entry_senha = self.widgets.entry(self, "*")
        self.entry_senha.grid(row=2, column=4, sticky="w", padx=(10,20), pady=(20,10))

        self.btn_salvar = self.widgets.button(self, texto="Salvar", comando=self.salvar_fisioterapeuta, cor="blue")
        self.btn_salvar.grid(row=3, column=1, sticky="e", padx=(20,10), pady=(20,10))
